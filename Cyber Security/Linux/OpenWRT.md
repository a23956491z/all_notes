...
如果不計冗餘架構，隨便連4臺24port的交換機，也能讓路由器處理近百臺設備呢
而且以這個規格，要處理幾十臺設備應該沒問題

而且也是因爲目前手上的交換機都是100M的，所以會造成頻寬的瓶頸，沒辦法發揮路由器1Gbps的功能。

另外這個OpenWRT也已經裝上OpenVPN，只需要下載openVPN導入profile就可以直接連回內網，雖然市面上有OpenVPN支援的路由器也是有，不過動輒3K 4K外，跑到3臺Server可能就是極限了，如果有更高的性能需求就又相當麻煩，不過用軟路由的方案，這個CPU看起來是遊刃有餘，更不用說如果需要更換10Gbps的網路，基本上整顆路由器就要換掉了，不過軟路由的話就只需要換掉網卡，其他的VPN、路由設定統統不需要改變，**可擴充性**實在不錯。

不過無線網路的部分會麻煩不少，最簡單的方案就是插上PCIE x1或USB的無線網卡，從幾百塊到2、3K都有，比較好的無線網卡基本上可以媲美消費級的頂規無線路由器了，當然如果能上企業級WAP，那效能更是無話可說了。
